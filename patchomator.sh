...
#
#   Version 1.0.6, 16-May-2023 Dan Snelson (@dan-snelson)
...
dialogListConfigurationOptions=(
	...
	--messagefont size=11
	--quitkey k
	--icon "SF=laptopcomputer color1=pink color2=blue"
)
...
PgetAppVersion() {
	...
	if [[ ${#appPathArray} -gt 0 ]]; then
		...
		if [[ ${#filteredAppPaths} -eq 1 ]]; then
			...
			swiftDialogUpdate "progresstext: Located $appName ($appversion)"
			sleep 2 # Give the user a couple o' seconds to observe
...
doInstallations() {
	...
	updateScriptLog "queuedLabelsArrayLength: $queuedLabelsArrayLength"
