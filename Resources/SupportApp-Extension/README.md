...
* **`aap_pending_apps_dialog.zsh`** - A thin wrapper that calls `appautopatch --pending-apps-dialog` (App Auto-Patch 3.7.0+). AAP shows a swiftDialog list of pending apps first (icon, name, "Current Version → New Version" subtitle) with "Install Now"/"Later" buttons - no deferral timer or menu. "Install Now" installs the pending updates in-process via AAP's standard install-now workflow, patching exactly the queue shown (no fresh discovery scan and no second `appautopatch` process); "Later" just closes the dialog. Either way the wrapper then re-runs `aap_pending_updates.zsh` to refresh the count. If there are zero pending apps when clicked, AAP shows a brief "You're all up to date!" message instead. Because the dialog is rendered by AAP itself, it automatically reuses the same managed/local preferences AAP uses for its own dialogs (`DialogIcon`, `UseOverlayIcon`, `BannerImage`/`BannerTitle`/`BannerHeight`, `AppTitle`, `DialogOnTop`, and the `display_string_version_current`/`display_string_version_new`/`display_string_there_are`/`display_string_deferral_button2` language overrides) so its look and wording stay consistent. The "Later" button's text can optionally be customized via the managed preference key `display_string_pendingapps_button_later`, alongside the other `dialogElements` entries - defaults to "Later" if not set. Best if you want end users to see what's pending and have a chance to defer before anything happens.
...
* For user-driven patching, set `WorkflowDisableRelaunch=true` with `WorkflowScheduledDiscovery=true`. AAP then refreshes this report on `DiscoveryFrequency` without automatically patching; the Support App tile remains the user-facing install entry point.
